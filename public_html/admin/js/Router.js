function HistoryStack(data, language) {
    ...
    this._dive = function() {
        ...
        while (i < this.stack.length) {
            ...
            if (Array.isArray(currentNode)) {
                ...
                currentNode = currentNode.children.filter(node => {
                    return node.name === this.stack[i];
                })[0] || currentNode.members.filter(node => {
                    return node.name === this.stack[i];
                })[0];
    ...
    this.breadcrumb = function () {
        ...
        this.stack.forEach((node, i) => {
            middle = middle.concat(`<li class="navigator-link${i === this.stack.length - 1 ? ' active' : ''}" onClick="router.navigate('${node.trim()}')">${node}</li>`)
        })
        ...
        if (data.type === "BUSINESS-UNIT" && data.head) {
            return `
                <section id="head" class="panel head" onClick="router.navigate('${data.head.name}')">
                    <div class="panel-heading">
                        <div class="row">
                            <div class="col-xs-10">
                                <p class='equipe_nome'>${data.head.name.replace(data.head.nickname, `<span class='equipe_laranja'>${data.head.nickname}</span>`)}</p>
                                <p class='equipe_email'>${this.language === 'pt' ? `Responável ${data.name}` : `Head of ${data.name}`}</p>
                            </div>
                            <div class="col-xs-2">
                                ${data.head.linkedin ? `<p class='text-left'><a href='${data.head.linkedin}' target='_blank'><img src='img_site/newLinkedinIcon.png' height="30px"></a></p>` : ""}
                            </div>
                        </div>
                    </div>
                    <div class="panel-body">
                        <img src=/img_site/${data.head.image} class="img-responsive hex-image"/>
                    </div>
                </section>
            `;
            ...
            <section id="head" class="panel head">
                <div class="panel-heading">
                    <div class="row">
                        <div class="col-xs-10">
                            <p class='equipe_nome'>${data.name.replace(data.nickname, `<span class='equipe_laranja'>${data.nickname}</span>`)}</p>
                            <p class='equipe_email'>${this.language === 'pt' ? `${data.title}` : `Head of ${data.title}`}</p>
                            ${data.email ? `<p class='equipe_email'><a class='equipe_email' href='mailto:${data.email}'>${data.email}</a></p>` : ""}
                        </div>
                        <div class="col-xs-2">
                            ${data.linkedin ? `<p class='text-left'><a href='${data.linkedin}' target='_blank'><img src='img_site/newLinkedinIcon.png' height="30px"></a></p>` : ""}
                        </div>
                    </div>
                </div>
                <div class="panel-body">
                    <img src=/img_site/${data.image} class="img-responsive hex-image"/>
                </div>
            </section>
            `;
        } else {
    ...
    this.child = function (child) {
        const description = child.description ? child.description : this.language === 'pt' ? "NENHUMA DESCRIÇÃO ENCONTRADA." : "NO DESCRIPTION FOUND.";
        return `
            ...
            <div class="panel panel-primary member" onClick="router.navigate('${member.name}')">
                ...
                <div class="panel-body row">
                    <div class="col-sm-4 member-image">
                        <img src=/img_site/${member.image} class="img-responsive"/>
                    </div>
                    <div class="col-sm-8 member-content" style="padding-left: 0; margin-left: 0;">
                        <div class="row">
                            <div class="col-xs-9">${member.title ? `<h4>${member.title}</h4>` : ""}</div>
                            <div class="col-xs-2">${member.linkedin ? `<p class='text-left'><a href='${member.linkedin}' target='_blank'><img src='img_site/newLinkedinIcon.png' height="30px"></a></p>` : ""}</div>
                        </div>
                        ${member.shortDescription ? `<p>${member.shortDescription}</p>` : ``}
                        ${member.email ? `<p class='equipe_email'><a class='equipe_email' href='mailto:${member.email}'>${member.email}</a></p>` : ""}
                    </div>
                </div>
            </div>
        `;
    ...
    this.render = function () {
        ...
        if (this.stack.length === 1) {
            ...
            <section id="hex" class='hex-container'>
                <svg class="hex hex-nw" version="1.1" xmlns="http://www.w3.org/2000/svg" width="170" height="195" viewbox="0 0 173.20508075688772 200" >
                    <g class ="hex-item">
                        <path d="M86.60254037844386 0L173.20508075688772 50L173.20508075688772 150L86.60254037844386 200L0 150L0 50Z" 
                            onClick="router.navigate('${data[0].name}')"/>
                        <text x="50%" y="55%" text-anchor="middle" fill="white" class="hex-text">${data[0].name}</text>
                    </g>
                </svg>
            
                <svg class="hex hex-ne" version="1.1" xmlns="http://www.w3.org/2000/svg" width="170" height="195" viewbox="0 0 173.20508075688772 200" >
                    <g class ="hex-item">
                        <path d="M86.60254037844386 0L173.20508075688772 50L173.20508075688772 150L86.60254037844386 200L0 150L0 50Z" 
                            onClick="router.navigate('${data[1].name}')"/>
                        <text x="50%" y="55%" text-anchor="middle" fill="white" class="hex-text">${data[1].name}</text>
                    </g>
                </svg>

                <div class="breakrow"></div>

                <svg class="hex hex-w" version="1.1" xmlns="http://www.w3.org/2000/svg" width="170" height="195" viewbox="0 0 173.20508075688772 200" >
                    <g class ="hex-item">
                        <path d="M86.60254037844386 0L173.20508075688772 50L173.20508075688772 150L86.60254037844386 200L0 150L0 50Z" 
                            onClick=""/>
                        <text x="50%" y="55%" text-anchor="middle" fill="white" class="hex-text"></text>
                    </g>
                </svg>
                ...
                <svg class="hex hex-sw" version="1.1" xmlns="http://www.w3.org/2000/svg" width="170" height="195" viewbox="0 0 173.20508075688772 200" >
                    <g class ="hex-item">
                        <path d="M86.60254037844386 0L173.20508075688772 50L173.20508075688772 150L86.60254037844386 200L0 150L0 50Z" 
                            onClick="router.navigate('${data[2].name}')"/>
                        <text x="50%" y="55%" text-anchor="middle" fill="white" class="hex-text">${data[2].name}</text>
                    </g>
                </svg>

                <svg class="hex hex-se" version="1.1" xmlns="http://www.w3.org/2000/svg" width="170" height="195" viewbox="0 0 173.20508075688772 200" >
                    <g class ="hex-item">
                        <path d="M86.60254037844386 0L173.20508075688772 50L173.20508075688772 150L86.60254037844386 200L0 150L0 50Z" 
                            onClick="router.navigate('${data[3].name}')"/>
                        <text x="50%" y="55%" text-anchor="middle" fill="white" class="hex-text">${data[3].name}</text>
                    </g>
                </svg>
